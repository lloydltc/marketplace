<x-layouts.app>
    <x-slot:title>Edit: {{ $product->title }}</x-slot:title>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <form method="POST" action="{{ route('vendor.products.update', $product) }}" class="space-y-5">
            ...
            <div class="bg-white border border-neutral-200 rounded-xl shadow-sm p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="5" required
                              class="w-full border border-neutral-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F0A820]/40 resize-y">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="sku" name="sku" value="{{ old('sku', $product->sku) }}" maxlength="50"
                           class="w-full border border-neutral-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#F0A820]/40">
                    @error('sku')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="bg-white border border-neutral-200 rounded-xl shadow-sm p-6 space-y-5">
                ...
                <div x-data="{ usd: {{ old('price_usd', $product->price_usd ?? 0) }}, rate: {{ old('exchange_rate', $product->exchange_rate ?? 0) }} }">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="price_usd" name="price_usd" x-model.number="usd"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="exchange_rate" name="exchange_rate" x-model.number="rate"
                                   value="{{ old('exchange_rate', $product->exchange_rate) }}"
        ...
        @include('partials.image-manager', [
            'images'       => $images,
            'uploadRoute'  => route('vendor.products.images.store', $product),
            'deleteRoute'  => 'vendor.products.images.destroy',
            'deleteParams' => ['product' => $product],
