...
<div class="bg-surface border border-base rounded-xl p-4 sm:p-5"
    ...
    <div class="flex items-center justify-between mb-3">
        <h3 class="text-body font-semibold text-strong">Find parts for your vehicle</h3>
        @if ($ctx->has())
            <form method="POST" action="{{ route('fitment.clear') }}">
                @csrf
                <button type="submit" class="text-caption text-[rgb(var(--text-muted))] hover:text-strong">Clear</button>
            </form>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('fitment.select') }}" class="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <select name="make_id" required x-model="makeId" @change="onMakeChange()"
                class="border border-base rounded-lg px-3 py-2 text-body-sm bg-surface focus:outline-none focus:ring-2 focus:ring-[rgb(var(--brand)/0.4)]">
