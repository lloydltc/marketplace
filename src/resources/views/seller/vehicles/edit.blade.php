<x-layouts.app>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <x-breadcrumbs class="mb-6" :items="[
            ...
            ['label' => $vehicle->displayTitle(), 'url' => route('seller.vehicles.show', $vehicle)],
            ['label' => 'Edit'],
        ]" />
        ...
        <form method="POST" action="{{ route('seller.vehicles.update', $vehicle) }}">
            @csrf
            @method('PUT')
            <x-stepper :steps="['Details', 'Specs', 'Pricing']" :start="$start">
                ...
                <x-slot:actions>
                    ...
                    <a href="{{ route('seller.vehicles.show', $vehicle) }}" class="self-center text-body-sm text-muted hover:text-[rgb(var(--text))]">Cancel</a>
                </x-slot:actions>
            </x-stepper>
        </form>
        ...
        @include('partials.image-manager', [
            'images'       => $images,
            'uploadRoute'  => route('seller.vehicles.images.store', $vehicle),
            'deleteRoute'  => 'seller.vehicles.images.destroy',
        ...
        <div class="mt-8 border-t border-line pt-8">
            ...
            <form method="POST" action="{{ route('seller.vehicles.destroy', $vehicle) }}"
